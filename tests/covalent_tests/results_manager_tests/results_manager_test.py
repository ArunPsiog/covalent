...
def test_cancel_with_single_task_id(mocker):
    mock_request_delete = mocker.patch(
        "covalent._api.apiclient.requests.Session.delete",
    )

    cancel(dispatch_id="dispatch", task_ids=1)

    mock_request_delete.assert_called_once()
    mock_request_delete.return_value.raise_for_status.assert_called_once()
