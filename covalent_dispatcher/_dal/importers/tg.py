...
def import_transport_graph(
    ...
    session.add_all(gid_job_record_map.values())
    ...
    session.add_all(electron_map.values())
    ...
    for _, asset_records_by_key in electron_asset_links.items():
        ...
        session.add_all(asset_records_by_key.values())
    ...
    session.add_all(meta_asset_associations)
